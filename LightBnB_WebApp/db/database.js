...
const getAllReservations = function (guest_id, limit = 10) {
  return pool
    .query(
      `SELECT reservations.*, properties.* FROM reservations JOIN properties ON reservations.property_id = properties.id WHERE reservations.guest_id = $1 LIMIT $2`,
      [guest_id, limit]
    )
    .then((result) => {
      return result.rows;
...
const getAllProperties = (options, limit = 10) => {
  ...
  // Check if no search options are provided
  if (
    (options.city === "" &&
    ...
    // If no search options provided, use a simple query to get all properties
    return pool
      .query(`SELECT * FROM properties LIMIT $1`, [limit])
      .then((result) => result.rows)
      .catch((err) => {
  ...
  // Handle filtering if search options are provided
  if (options.city) {
  ...
  if (options.owner_id) {
    return pool
      .query(`SELECT * FROM properties WHERE owner_id = $1 LIMIT $2`, [
        options.owner_id,
        limit,
      ])
      .then((result) => result.rows)
